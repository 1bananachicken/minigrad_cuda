...
class Conv2d:
    ...
    def forward(self, x: np.ndarray) -> np.ndarray:
        self.x = x
        _conv = _Conv(self.padding, self.stride)
        out_h = (x.shape[2] - self.kernel_size + 2 * self.padding) // self.stride + 1
        out_w = (x.shape[3] - self.kernel_size + 2 * self.padding) // self.stride + 1
    ...
    def backward(self, dout: np.ndarray) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
        _conv = _Conv(self.padding, self.stride)
        out_h = (self.x.shape[2] - self.kernel_size + 2 * self.padding) // self.stride + 1
        out_w = (self.x.shape[3] - self.kernel_size + 2 * self.padding) // self.stride + 1
                ...
                for j in range(self.in_channels):
                    for x in range(out_h):
                        for y in range(out_w):
                            ...
                            x_end = min(x_start + self.kernel_size, self.x.shape[2])
                            y_start = y * self.stride
                            y_end = min(y_start + self.kernel_size, self.x.shape[3])
                            region = self.x[n, j, x_start:x_end, y_start:y_end]
                            kernel_portion = self.kernel.data[i, j, :x_end - x_start, :y_end - y_start]

                            dk[i, j, :x_end - x_start, :y_end - y_start] += dout[n, i, x, y] * region
                            dx[n, j, x_start:x_end, y_start:y_end] += dout[n, i, x, y] * kernel_portion
